...
async function testContractCalls(t: test.Test, createClient: () => Client) {
  ...
  await contract.callAsync<void>(client.caller, 'SetMsg', msg)

  const retVal = await contract.callAsync<MapEntry>(client.caller, 'SetMsgEcho', msg, new MapEntry())
  ...
  msg.setValue('')
  const result = await contract.staticCallAsync<MapEntry>(client.caller, 'GetMsg', msg, new MapEntry())
